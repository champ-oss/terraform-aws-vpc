resource "aws_subnet" "public" {
  ...
  tags              = merge({ Name : "${var.git}-public-${count.index}" }, { Type : "Public" }, local.tags, var.tags, var.public_subnet_tags)
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = data.aws_availability_zones.this.names[count.index]
  tags              = merge({ Name : "${var.git}-private-${count.index}" }, { Type : "Private" }, local.tags, var.tags, var.private_subnet_tags)
}